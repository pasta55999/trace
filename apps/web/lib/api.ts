...
export const api = {
  ...
  status: () => req<Status>("/investigation/status"),
...
export interface AssetRow { asset_id: string; description: string; borrower: string; borrower_ar: string; sector: string; precision: string; status: string; depth_above_floor_m: Value; outstanding_allocated_aed: number; physical_damage_total_aed: Value; interruption_cost_aed: Value; insured_loss_aed: Value; collateral_value_sensitivity_aed: Value; flood_depth_m: Value; ref: string }
export interface Run { run_id: string; label: string; scenario: { label_en: string; label_ar: string }; assets: AssetRow[]; aggregation: { portfolio_outstanding_aed: number; concentration: { assets_in_footprint: string[]; borrowers: string[]; sectors: string[]; outstanding_in_footprint_aed: number; share_of_portfolio_outstanding: number; physical_damage_total_aed: Value; insured_loss_total_aed: Value }; diversification_warning_en: string; diversification_warning_ar: string } }
...
export interface Measure { id: string; name_en: string; name_ar: string; capex_aed: number }
